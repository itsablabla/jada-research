export const jaJP = {
  ...
  mcp: {
    title: "MCPツール",
    description: "外部MCP（Model Context Protocol）サーバーを接続して、チャット会話中にAIが外部ツールにアクセスできるようにします。接続されたサーバーのツールは、ノートブックでのチャット時に自動的に利用可能になります。",
    addServer: "サーバーを追加",
    addFirstServer: "最初のサーバーを追加",
    editServer: "MCPサーバーを編集",
    addServerTitle: "MCPサーバーを追加",
    deleteServer: "MCPサーバーを削除しますか？",
    deleteServerDesc: "サーバーが削除され、そのツールはチャットで使用できなくなります。",
    name: "名前",
    namePlaceholder: "例：Composio、Garza MCP",
    url: "URL",
    ...
    noServersDesc: "MCPサーバーを追加して、AIがメール、ファイル管理などの外部ツールにアクセスできるようにします。",
    testConnection: "接続テスト",
    connected: "接続済み — {count}個のツールが利用可能",
    failed: "失敗",
    showTools: "{count}個のツールを表示",
    hideTools: "ツールを非表示",
    active: "有効",
    disabled: "無効",
    cancel: "キャンセル",
